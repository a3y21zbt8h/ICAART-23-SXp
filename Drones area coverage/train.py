...
#  Compute a training of an agent or a windagent
#
#  Input: filepaths of logs, models and logfile (String), environment (DroneAreaCoverage), agents (Agent list),
#  NN and target NN (DQN), learning rate (float), epsilon values limits and it's decay rate (float), type of device
...
def agentTrain(log_dir, model_dir, filename, env, agents, net, tgt_net, buffer, learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move, behaviour=None):
    # Initialization
...
def correlTrain(log_dir, model_dir, filename, env, agents, wind_agents, net, wind_net, wind_tgt_net, buffer, learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move):
    ...
    while True:
        ...
        #  Step and update observation ---------------------------------------------------------------------------------
        states, temp_states, new_states, dones, _ = env.step(agents, actions, move=move)
...
#  Save informations into lists and a file, reset agents and store the current NN when a better cumulative reward is
#  reached. save() can stop the training process and it's called a the end of an episode.
#
#  Input: directory path to store NN (String), current cumulative reward, the best one reached and the maximum one
#  (float), environment (DroneAreaCoverage), agents (Agent list), current NN (DQN), file to write infos,
...
#
#  Output: end of training process (boolean), best current cumulative reward (float), file to write infos, list of
...
#  ended time horizon
#  Input: filenames (String list), datas to store in filenames (float list list), datas labels (String list)
...
if __name__ == "__main__":
    ...
    if not windless:
        ...
        #  Agents train ------------------------------------------------------------------------------------------------
        losses, rewards, timeH, best_reward, filename_last_net = agentTrain(agent_dir_log, agent_dir_model, filename, env, agents, net, tgt_net, buffers[0], learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move)
        ...
        if not correlated:
            ...
            losses, rewards, timeH, best_reward, _ = agentTrain(hostile_dir_log, hostile_dir_model, filename, env, wind_agents[0], h_net, h_tgt_net, buffers[1], learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move, behaviour="hostile")
            datas = [losses, rewards, timeH]
            ...
            losses, rewards, timeH, best_reward, _ = agentTrain(favorable_dir_log, favorable_dir_model, filename, env, wind_agents[1], f_net, f_tgt_net, buffers[2], learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move, behaviour="favorable")
            datas = [losses, rewards, timeH]
        ...
        #  Use the Agents learnt policy
            ...
            env.initObs(agents, random_starting_position)
            losses, rewards, timeH, best_reward = correlTrain(hostile_dir_log, hostile_dir_model, filename, env, agents, wind_agents[0], net, h_net, h_tgt_net, buffers[1], learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move)
            ...
            env.initPos(agents, random_starting_position)
            env.initObs(agents, random_starting_position)
            losses, rewards, timeH, best_reward = correlTrain(favorable_dir_log, favorable_dir_model, filename, env, agents, wind_agents[1], net, f_net, f_tgt_net, buffers[2], learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move)
            datas = [losses, rewards, timeH]
            saveInfos(favorable_filenames, datas, ylabels, favorable_dir_log)
    #  Train of Agents
    else:
        losses, rewards, timeH, best_reward, _ = agentTrain(agent_dir_log, agent_dir_model, filename, env, agents, net, tgt_net, buffers[0], learning_rate, epsilon_final, epsilon_start, epsilon_decay, device, time_horizon, replay_start, sync_target, batch_size, gamma, time_limit, random_starting_position, ddqn, move)
        datas = [losses, rewards, timeH]
        saveInfos(filenames, datas, ylabels, agent_dir_log)
